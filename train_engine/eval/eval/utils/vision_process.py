...
def get_resize_output_image_size(height, width, shortest_edge, longest_edge):
    """
    Calculates the output image size after resizing, preserving aspect ratio.
    """
...
def fps_indices(
    ...
    if len(indices) % 4 != 0:
        indices = indices + [indices[-1]] * (4 - len(indices) % 4)

    if len(indices) < min_num_frames:
        indices = uniform_indices(min_num_frames, total_frames)
    return indices
    ...
    **kwargs,
    ...
    if sample_type == "uniform":
        ...
        if num_frames == "auto":
            ...
            # vid.release()
            vid_name = src_path.split("/")[-1][:-4]  # remove .mp4
            height = (
                video_size_list[vid_name]["height"]
                if video_size_list
                else vr.get_height()
            )
            width = (
                video_size_list[vid_name]["width"]
                if video_size_list
                else vr.get_width()
            )
            height, width = get_resize_output_image_size(
                height, width, img_shortest_edge, img_longest_edge
            )
            num_patches = int(
                (height // model_patch_size) * (width // model_patch_size)
            )
            ...
            width = (
                video_size_list[vid_name]["width"] if video_size_list else vr.get_width
            )
            img_shortest_edge = kwargs["img_shortest_edge"]
            img_longest_edge = kwargs["img_longest_edge"]
            height, width = get_resize_output_image_size(
        ...
        indices = uniform_indices(int(num_frames), total_frames)
        durations = [idx / input_fps for idx in indices]
        frames = vr.get_batch(indices).asnumpy()  # (T, H, W, C), np.uint8
        frames = [
            (
                Image.fromarray(frame).resize((int(width), int(height)), resample=3)
                if width and height
        ...
        min_num_frames = kwargs.pop("min_num_frames", 1)
        indices = fps_indices(
            input_fps, total_frames, output_fps, max_num_frames, min_num_frames
        )
        durations = [idx / input_fps for idx in indices]
        frames = vr.get_batch(indices).asnumpy()  # (T, H, W, C), np.uint8
    ...
    if sub_path is None:
        ...
        subtitles = load_subtitle(
            sub_path, indices=indices, fps=float(vr.get_avg_fps())
        )
...
def _read_video_decord_w_timestamp(
    ...
    # print("video_start", video_start, "video_end", video_end, "start_frame", start_frame, "end_frame", end_frame, 'total_frames', total_frames)

    if end_frame == start_frame:
        end_frame = start_frame + 1
    if end_frame < start_frame or end_frame > total_frames:
        raise ValueError("Video timestamps are error!")
...
def fetch_video(
    ...
    if isinstance(ele["video"], str):
        ...
        # video, sample_fps = _read_video_decord_w_timestamp(ele)
...
def process_vision_info_v3(
    ...
    for vision_info in vision_infos:
        if "image" in vision_info or "image_url" in vision_info:
            ...
            video_input, video_sample_fps = fetch_video_v3(
                vision_info, return_video_sample_fps=True
            )
            video_sample_fps_list.append(video_sample_fps)
            video_inputs.append(video_input)
