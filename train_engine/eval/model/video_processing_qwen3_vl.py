    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        for shape, stacked_videos in grouped_videos.items():
            ...
            # Ensure grid_h and grid_w are divisible by merge_size
            # This is a fallback to handle cases where process_vision_info returns
            # dimensions that don't satisfy the merge_size requirement
            if grid_h % merge_size != 0 or grid_w % merge_size != 0:
                # Calculate the target grid dimensions that are divisible by merge_size
                target_grid_h = (grid_h // merge_size) * merge_size
                target_grid_w = (grid_w // merge_size) * merge_size

                # Calculate the target height and width
                target_height = target_grid_h * patch_size
                target_width = target_grid_w * patch_size

                # Crop the video to the target size
                patches = patches[:, :, :, :target_height, :target_width]
                resized_height, resized_width = target_height, target_width
                grid_h, grid_w = target_grid_h, target_grid_w
